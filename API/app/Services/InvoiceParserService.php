...
class InvoiceParserService
{
    ...
    /**
     * Extrai transações de um arquivo PDF (Mercado Pago)
     */
public function parsePdf(string $filePath): array
    {
        ...
        try {
            ...
            // Usa o caminho completo já resolvido
            $text = $this->extractPdfText($fullPath);
            
            // Extrai o ano da fatura do PDF
    ...
    /**
     * Extrai transações de uma seção específica do PDF
     */
    private function extractTransactionsFromSection(string $text, int $sectionPos, ?int $invoiceYear): array
    {
        $transactions = [];
        ...
        foreach ($matches as $match) {
            ...
            // Se o mês é 12 e a fatura é de dezembro, pode ser do ano anterior para algumas transações
